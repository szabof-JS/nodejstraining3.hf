...
// MODIFICATIONS HERE: 8 new lines
// simple logger using express middleware
var debugStepLogger = require('debug')('myservice.logger');
function stepLogger(req, res, next) {
  debugStepLogger(`<debugStepLogger> request received to ${req.originalUrl}`); // req.path (valamiért üres volt)  req.app (csak nevesített függvénynél lehet jó)
  next();
}
// add logger midlleware to all /service* routes
app.use("/step[12]", stepLogger);
